...
- **`fallow init --hook-user` installs a user-scope Claude Code Stop hook.** Runs `fallow audit` after every Claude Code turn that edits TS/JS files, across all projects on the machine. Includes a 3-strike anti-loop guard (after three consecutive blocked turns Claude is instructed to stop and ask the user instead), graceful degradation when `fallow` is missing or below the minimum version, `FALLOW_HOOK_DRY_RUN=1` mode for a safe one-week trial rollout before going live, and `fallow init --hook-user --uninstall` for full reversal. Existing Stop hooks in `~/.claude/settings.json` are preserved. See `docs/hooks/user-scope-stop-hook.md`.
